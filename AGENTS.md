...
- 扩展**不主动调用任何站点后端接口**（不 fetch、不 XHR、不 WebSocket 到站点）。
- 作物图标允许通过 `<img>` 加载 `https://cdk.hybgzs.com/farm/crops/` 下的静态 PNG；校验地址并限制图片 CSP，失败时依次回退到随包作物图标和通用图标，不为图标查询接口。
- 不执行页面操作（不点击、不提交、不导航）。
- 数据只来自被动 hook 页面自身的请求响应；页面数据与本地记录冲突时以页面数据为准。
- 成熟时间可去重压缩数量。
- `seedImage` 随作物数据保存并传到面板；基础路径追加 `_s4.png` 得到成熟图标。旧记录缺少 `seedImage` 时按 `seedId` 推导，面板倒计时刷新复用图片节点。
